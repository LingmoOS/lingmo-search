...
LingmoSearchGui::LingmoSearchGui(int &argc,
                             char *argv[],
                             ...
                             QtSingleApplication (applicationName + display, argc, argv)
{
    ...
    if (!this->isRunning()) {
        ...
        m_mainWindow = new LingmoSearch::MainWindow();
        m_dbusService = new LingmoSearch::LingmoSearchDbusServices(m_mainWindow, this);
...
LingmoSearchGui::~LingmoSearchGui()
{
    if(m_mainWindow) {
        delete m_mainWindow;
...
void LingmoSearchGui::parseCmd(QString msg, bool isPrimary)
{
