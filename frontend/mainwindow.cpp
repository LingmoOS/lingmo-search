 ...
 *
 * Authors: zhangjiaping <zhangjiaping@lingmoos.cn>
 * Modified by: zhangpengfei <zhangpengfei@lingmoos.cn>
 *
 */
...
void MainWindow::centerToScreen(QWidget* widget) {
    if(!widget)
        return;
    KWindowSystem::setState(this->winId(),NET::SkipTaskbar | NET::SkipPager | NET::SkipSwitcher);
    QRect desk_rect = LingmoQuick::WindowProxy::currentScreen()->geometry();
...
void MainWindow::initTimer() {
    ...
    connect(m_askTimer, &QTimer::timeout, this, [ = ]() {
        ...
        if(this->isVisible() && !m_isIndexSearch) {
            ...
            kdk::LingmoStyleHelper::self()->removeHeader(m_askDialog);
            m_askDialog->show();
            //设置跳过多任务视图
