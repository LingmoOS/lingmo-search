...
LingmoSearchService::LingmoSearchService(int &argc, char *argv[], const QString &applicationName)
...
LingmoSearchService::~LingmoSearchService()
{
    if(m_quickView) {
        delete m_quickView;
        m_quickView = nullptr;
...
void LingmoSearchService::parseCmd(const QString& msg, bool isPrimary)
{
...
void LingmoSearchService::loadMonitorWindow()
{
    if(!m_monitor) {
        m_monitor = new Monitor(m_indexScheduler, this);
    }
...
void LingmoSearchService::showMonitorState()
{
...
QUrl LingmoSearchService::nodeUrl()
{
